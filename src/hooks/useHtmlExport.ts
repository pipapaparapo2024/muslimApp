...
export const useHtmlExport = () => {
  ...
  // Генерация HTML контента для QnA
  const generateQnaHtmlContent = (): string => {
    return `
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
  ...
  // Генерация HTML контента для Scanner
  const generateScannerHtmlContent = (): string => {
    return `
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
  ...
  const exportHtml = async (options: ExportOptions): Promise<string> => {
    ...
    try {
      ...
      // Генерируем HTML контент в зависимости от типа
      if (options.type === "qna") {
        htmlContent = generateQnaHtmlContent();
      } else {
        htmlContent = generateScannerHtmlContent();
      }
