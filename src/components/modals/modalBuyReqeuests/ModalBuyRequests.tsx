...
export const BuyRequestsModal: React.FC<BuyRequestsModalProps> = ({
  ...
  const getPrices = (optionLabel: string) => {
    const option = requestOptions.find((opt) => opt.label === optionLabel);
    if (!option) return { ton: 1, stars: 1, quantity: parseInt(optionLabel) || 10, productId: null, currencyId: null };

    const tonCurrency = getPriceByProductId(option.product.id, "TON");
    const starsCurrency = getPriceByProductId(option.product.id, "XTR");

    return { ton: tonCurrency?.priceAmount || 1, stars: starsCurrency?.priceAmount || 1, quantity: option.quantity, productId: option.product.id, currencyId: starsCurrency?.id };
  };

  const prices = selectedRequests ? getPrices(selectedRequests) : { ton: 0, stars: 0, quantity: 0, productId: null, currencyId: null };
  ...
  const handleTonPurchase = async () => {
    ...
    setIsProcessingTon(true);
    try {
      const result = await payWithTon({ amount: prices.ton, type: "requests", quantity: prices.quantity, productId: prices.productId });
      if (result.status === "success") {
        onClose();
      } else if (result.status === "rejected") {
        alert("Платёж отклонён");
      } else {
        alert("Ошибка оплаты");
      }
    } catch {
      ...
      <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
        ...
        <div className={styles.priceBlocks}>
          ...
          >
            <div className={styles.priceText}>
              <img src={ton} alt="TON" width="24" height="24" />
              <div className={styles.priceValueTon}>{(prices.ton / 10e9).toFixed(2)}</div>
            </div>
